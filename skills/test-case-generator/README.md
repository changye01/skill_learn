...
7. 参考 `references/quality-rubric.md` 做一轮内容质量自查
...
2. 再提供对应的 `reference-pack`（`tables/*.sql` + `table_samples/*.csv`）
3. 如存在派生字段、状态映射、历史修复，再补充技术方案
4. 由 `test-data-generator` 保留 `结构化测试用例.md` 作为基线稿，并进一步输出按 `TC` 展开的 `测试执行清单.md`
