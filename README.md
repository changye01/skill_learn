...
2. 确认后先保存 `..._测试场景地图.md`
3. 再输出并确认 `结构化测试用例` 草稿
4. 确认后保存 `..._结构化测试用例.md` 和 `..._表格版用例.csv`
5. 使用 `python3 validate_cases.py` 校验 `Markdown + CSV` 一致通过
...
- `example_data/2026-03-17=!【采购-订单模块】M4X订单管理：查询条件及列表字段调整、新增分配采购员功能/【采购-订单模块】M4X订单管理：查询条件及列表字段调整、新增分配采购员功能_表格版用例.csv`
...
4. 第二阶段确认后保存 `..._结构化测试用例.md + ..._表格版用例.csv`，并使用 `python3` 运行校验脚本确认结构完整、CSV 与 Markdown 一致。
...
- `【采购-订单模块】M4X订单管理：查询条件及列表字段调整、新增分配采购员功能_表格版用例.csv`
